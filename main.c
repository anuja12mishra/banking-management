...
account_no:
    ...
    while (fscanf(ptr, "%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d %s\n", &add.acc_no, add.name, &add.dob.month, &add.dob.day, &add.dob.year, &add.age, add.address, add.citizenship, &add.phone, add.acc_type, &add.amt, &add.deposit.month, &add.deposit.day, &add.deposit.year, add.password) != EOF) {
    ...
    while (fscanf(old, "%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d %s", &add.acc_no, add.name, &add.dob.month, &add.dob.day, &add.dob.year, &add.age, add.address, add.citizenship, &add.phone, add.acc_type, &add.amt, &add.deposit.month, &add.deposit.day, &add.deposit.year, add.password) != EOF) {
        if (add.acc_no == transaction.acc_no) {
            ...
            if (strcmp(pass, add.password) != 0) {
                printf("\nIncorrect password! Transaction aborted.\n");
                fclose(old);
                fclose(newrec);
                remove("new.dat");
...
void close(void) {
    printf("\n\n\n\nThis C Mini Project is developed by Anuja, Archita and Pradyumna !");
}
